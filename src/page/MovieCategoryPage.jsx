    ...
    <div className="background-color min-h-[100vh]">
      ...
      <div className="pt-25 relative mx-4 2xl:mx-[8rem]">
        ...
        <div className="flex gap-2 justify-center items-center mt-6 text-[.8rem]">
            ...
            <button onClick={() => currentPage > 1 && navigate(`/movie/${category}/${currentPage - 1}`)}disabled={currentPage === 1}className={`px-4 py-2 rounded-[3px] ${currentPage === 1 ? "bg-gray-500 cursor-not-allowed" : "bg-primary-color text-white cursor-pointer"}`}>Prev</button>

            {getPagination(1, totalPages, currentPage, 3).map((page, index) => (
                <button
                key={index}
                onClick={() => page !== "..." && navigate(`/movie/${category}/${page}`)}
            ...
            <button onClick={() => currentPage < totalPages && navigate(`/movie/${category}/${currentPage + 1}`)}disabled={currentPage === totalPages} className={`px-4 py-2 rounded-[3px] ${currentPage === totalPages ? "bg-gray-500 cursor-not-allowed" : "bg-primary-color text-white cursor-pointer"}`}>Next</button>
        </div>
