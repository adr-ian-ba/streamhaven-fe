    ...
    <div className='background-color relative overflow-hidden'>
      <Navigation />
        <div className='absolute top-0 left-0 w-screen min-h-[40rem]'>
            ...
            <img className='object-cover w-full min-h-[50rem] lg:h-auto' src={movieInfo.backdrop_path} alt="" />
        </div>
        <div className='relative mx-auto px-4 max-w-[60rem] min-h-[100dvh]'>
            <div className='mt-20'></div>
            <div onClick={goBack} className='flex items-center gap-2 mb-[1rem] p-1 cursor-pointer'>
            ...
            <div className='flex gap-2 text-sm'>
                <img className='hidden sm:block max-w-[12rem] rounded-[3px]' src={movieInfo.poster_path} alt="" />
                <div>
                    <h1 className='text-2xl'>{movieInfo.original_name || movieInfo.title} {ss ? `SS${ss}:EP${ep}` : "" }</h1>
                    <p className='max-w-[40rem] mb-2'>{movieInfo.overview}</p>
                    <p className='mb-2'>{movieInfo.vote_average} / {movieInfo.vote_count}</p>
                    <p className='mb-2'>{movieInfo.release_date} • {movieInfo.runtime}m</p>
            ...
            {movieInfo.seasons && (
                <div className='flex flex-wrap gap-4 mt-2'>
                    {movieInfo.seasons.filter((eachSeason)=> eachSeason.season_number != 0).map((eachSeason, index) => (
                        <div key={index} onClick={()=> changeSeason(eachSeason.season_number)} className={`${ss == eachSeason.season_number ? "border-primary border" : "border-gray-400 border"} cursor-pointer overflow-hidden relative rounded-[5px] text-sm w-[8rem] h-[5rem] border-gray-400 grid place-items-center p-1`}>
                            <div className="h-full w-full bg-black opacity-70 absolute inset-0 z-20"></div>
                            <img src={eachSeason.poster_path} className='z-10 w-full h-full absolute top-0 left-0 object-cover' alt="" />

                            <h1 className={`${ss == eachSeason.season_number ? "color-primary" : ""} z-30`}>
                                ss{eachSeason.season_number} : {eachSeason.name} : {eachSeason.episode_count}ep
                            </h1>
                        </div>
                    ))}
                </div>
            ...
            {movieInfo.episodes && (
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mt-4 gap-2'>
                    {movieInfo.episodes.map((eachEpisode, index)=>(
                        <div key={index} onClick={()=>changeEpisode(eachEpisode.episode_number)} className={`${eachEpisode.episode_number == ep ? "color-primary" : ""} border p-2 cursor-pointer`}>
                            <p className={`${eachEpisode.episode_number == ep ? "color-primary" : ""} truncate`}>EP{eachEpisode.episode_number} {eachEpisode.name}</p>
                        </div>
                    ))}
                </div>
            )}
