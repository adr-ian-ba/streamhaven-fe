...
//     setTimeout(() => {
//       const movie = trendingMovies[spotlightIndexRef.current % trendingMovies.length];
...
//         spot: Math.floor(Math.random() * 10) + 1,
//       });

//       setIsFading(false);
//       spotlightIndexRef.current++;
    ...
    return (
        <div className="background-color">
            ...
            <div className="relative min-h-[100dvh] overflow-x-hidden overflow-y-hidden">
                {spotLight && (
                  <div
                    className={`absolute top-0 left-0 w-screen h-[80vh] transition-opacity duration-1000 ${
                  ...
                  >
                    ...
                    <div className="absolute top-0 left-0 w-screen h-[80vh]">
                        ...
                        <div className="absolute top-[28%] sm:top-[30%] mx-4 2xl:mx-[8rem] z-20">
                            <h1 className="line-clamp-2 text-[2.5rem] sm:text-[3rem] font-bold text-white break-words whitespace-normal w-full max-w-screen-xl mx-auto leading-10 mb-[0.5rem]">
                                {spotLight.title}
                            </h1>
                            <p className="text-[.8rem] sm:text-sm max-w-[35rem] color-white overflow-hidden text-ellipsis line-clamp-4 mb-[.5rem]">
                                {spotLight.overview}
                            </p>
                            ...
                            <div className="flex items-center gap-1 mb-[.5rem]">
                                ...
                                <h5>
                                    {spotLight.vote_count} - # {spotLight.spot}
                                </h5>
                            </div>
                      ...
                      <Genre genre={eachGenre.name || ""} key={index} />
                            ...
                            <button onClick={() => handleClick("MV")} className="watch-now">
                                Watch Now
                            </button>
                        </div>
                    </div>
                ...
                <div className="relative mx-4 2xl:mx-[8rem] sm:pt-[60vh] pt-[67vh]">
                    <div className="flex items-center gap-4 mb-[1rem]">
                        ...
                        <div className="flex h-full gap-2">
                            <div
                                ...
                                className="bg-primary-color p-1.5 rounded-[3px] cursor-pointer"
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    fill="none"
                    ...
                    <div
                        ref={topDesktopRef}
                        className="top-desktop flex gap-[12rem] overflow-x-scroll overflow-y-visible h-[18rem] hidden-scrollbar"
                    >
                        {trendingMovies.slice(0, 10).map((eachMovie, index) => (
                            <div
  onClick={() => changeSpotLight(eachMovie, index + 1)}
  className="relative cursor-pointer group"
  key={eachMovie.id}
>
  <div
    className={`absolute top-0 ${index + 1 === 10 ? "left-30" : "left-20"} overflow-x-visible overflow-y-visible`}
  >
    <div className="rounded-[3px] overflow-hidden">
      <img
        src={`${eachMovie.poster_path}`}
        className="h-full w-full transition-all duration-200 group-hover:scale-110 cursor-pointer"
        alt=""
        onClick={(e) => {
          e.stopPropagation();
          handleClick(eachMovie.id, "MV");
        }}
      />
    </div>
    <p className="mt-[.5rem] text-sm text-center w-[10rem] truncate">
      {eachMovie.title}
    </p>
  </div>
  ...
  <h1
    className={`${
      spotLight.spot === index + 1 ? "color-primary" : ""
    } text-[10rem] leading-[15rem] w-[5rem] transition-all duration-200`}
  >
    {index + 1}
  </h1>
</div>
                    ...
                    <div
                        ref={topMobileRef}
                        className="top-mobile flex gap-[1rem] overflow-auto h-[18rem] hidden-scrollbar"
                    >
                        {trendingMovies?.slice(0, 10).map((eachMovie, index) => (
                            <div
  onClick={() => changeSpotLight(eachMovie, index + 1)}
  ...
  <div className="w-[2rem] h-[2rem] rounded-bl-[5px] bg-primary-color rounded-tr-[3px] absolute right-0 top-0 grid place-items-center">
    <h5 className="text-[.8rem]">#{index + 1}</h5>
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <MovieGrid
                    movies={pick === "movie" ? trendingMovies : trendingSeries}
                    className="relative mx-4 2xl:mx-[8rem] mt-[2rem] mb-[14rem]"
                >
                    <div className="mb-[1rem]">
                        <h1 className="text-[2rem] font-bold mb-[.5rem]">Trending</h1>
                        <div className="flex items-center gap-4">
                            <p
                                className={`border px-4 py-1 rounded-[3px] cursor-pointer ${
                                    pick === "movie" ? "bg-primary-color border-primary" : ""
                                }`}
                            ...
                            <p
                                className={`border px-4 py-1 rounded-[3px] cursor-pointer ${
                                    pick === "series" ? "bg-primary-color border-primary" : ""
                                }`}
